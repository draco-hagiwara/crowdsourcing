...
{form_open('/orderlist/detail00/' , 'name="detailForm" class="form-horizontal"')}
    <table class="table table-striped table-hover">
        ...
        <tbody>
            <tr>
                <td>
                    <button type="submit" class="btn btn-success btn-xs" name="pjid_uniq" value="{$list.pj_id}">確認</button>
                </td>
                <td>
                  {if $list.pj_deliver_flg == FALSE}
                    {if $list.pj_status == "0"}<font color="#ffffff" style="background-color:aqua">準 備 中</font>
                    {elseif $list.pj_status == "1"}<font color="#ffffff" style="background-color:navy">公 開 中</font>
                    ...
                    {/if}
                  {else}
                    <font color="#ffffff" style="background-color:red">納 品 済</font>
                  {/if}
                  <br>
                  {if $list.pj_entry_status == "1"}<font color="#ffffff" style="background-color:hotpink">ｴﾝﾄﾘｰ中</font>
                  {else}<font color="#ffffff" style="background-color:lime">ｴﾝﾄﾘｰ無</font>
                  {/if}
                </td>
                <td>
                    {$list.pj_en_id}
                ...
                <td>
                    ...
                    {$list.pj_en_entry_date|date_format:"%Y-%m-%d %H:%M"}
                </td>
                <td>
                    {$list.pj_en_delivery_date|date_format:"%Y-%m-%d"}<br>
