...
class Orderlist extends MY_Controller
{
	...
	public function index()
	{
		...
		list($listall, $countall) = $this->pj->get_postinglist($flash_data['c_memID'], $tmp_inputpost, $tmp_per_page, $tmp_offset);
	...
	public function search()
	{
		...
		if ($this->input->post('submit') == '_submit')
		{
			// セッションをフラッシュデータとして保存
			$data = array(
					'c_pj_pe_id'        => $this->input->post('pj_pe_id'),
			...
			// セッションからフラッシュデータ読み込み
			$tmp_inputpost['pj_pe_id']        = $this->session->userdata('c_pj_pe_id');
	...
	public function detail00()
	{
		...
		// SELECT項目 初期値セット
		$this->_form_item_set00($get_data[0]['pj_pe_cl_id']);
		...
		$get_data[0]['pj_pe_entry_date'] = date('Y-m-d',     strtotime($get_data[0]['pj_pe_entry_date']));
