...
class Pay_list extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $tmp_inputpost['cp_cl_id'] = $this->session->userdata('c_memID');

        list($pay_list, $pay_countall) = $this->cl->get_paylist($tmp_inputpost, $tmp_per_page, $tmp_offset);
        ...
        $this->view('client/pay_list/index.tpl');
    ...
    private function _get_Pagination($entry_countall, $tmp_per_page)
    {
        ...
        $config['base_url']       = base_url() . '/pay_list/search/';       // ページの基本URIパス。「/コントローラクラス/アクションメソッド/」
        $config['per_page']       = 12;                                     // 1ページ当たりの表示件数。
