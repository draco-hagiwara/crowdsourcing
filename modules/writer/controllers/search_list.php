...
class Search_list extends MY_Controller
{
    ...
    // 「気になるリスト」検索専用 一覧表示
    //       ～/writer/my_favorite/
    public function search_favorite()
    {

        // セッションからフラッシュデータ読み込み
        $tmp_inputpost['pj_id'] = $this->session->userdata('w_pj_id');

        $tmp_inputpost['pj_order_title'] = '';
        $tmp_inputpost['pj_genre01'] = '0';
    ...
    public function data_entry()
    {
        ...
        if ($post_data['submit'] == '_submit')
        {
            ...
            $result = $this->fav->insert_favorite($flash_data['w_memID'], $flash_data['w_pj_id'], $post_data);

            $_res_mess = '「気になるリスト」へ追加されました。';
