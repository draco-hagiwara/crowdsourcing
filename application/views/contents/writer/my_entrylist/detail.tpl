...
<ul class="nav nav-tabs">
  {if $order_no == '00'}<li role="presentation" class="active">{else}<li role="presentation">{/if}<a href="/my_entrylist/detail00">仕事内容</a></li>
  {if $order_no == '01'}<li role="presentation" class="active">{else}<li role="presentation">{/if}<a href="/my_entrylist/detail01/">仕事詳細１</a></li>
  {if $job_cnt == 2}
    {if $order_no == '02'}<li role="presentation" class="active">{else}<li role="presentation">{/if}<a href="/my_entrylist/detail02/">仕事詳細２</a></li>
  {/if}
  {if $job_cnt == 3}
    {if $order_no == '02'}<li role="presentation" class="active">{else}<li role="presentation">{/if}<a href="/my_entrylist/detail02/">仕事詳細２</a></li>
    {if $order_no == '03'}<li role="presentation" class="active">{else}<li role="presentation">{/if}<a href="/my_entrylist/detail03/">仕事詳細３</a></li>
  {/if}
</ul>
...
{form_open('my_entrylist/data_save/' , 'name="OrderForm" class="form-horizontal"')}
  ...
  <div class="form-group">
    ...
    <div class="col-sm-8">
      {form_input('rep_title' , set_value('rep_title', $order_info.rep_title) , 'class="form-control" placeholder="タイトルを入力してください"')}
      {if form_error('rep_title')}<span class="label label-danger">Error : </span><label><font color=red>{form_error('rep_title')}</font></label>{/if}
    </div>
    <div class="col-sm-1">
      <button type="submit" class="btn btn-success btn-xs" name="check" value="">Check</button>
    </div>
  </div>
  ...
  <div class="form-group">
    ...
    <div class="col-sm-8">
      ...
      {$attr['rows'] = 20}
      {form_textarea($attr , set_value('rep_text_body', $order_info.rep_text_body) , 'class="form-control" placeholder="本文を入力してください"')}
